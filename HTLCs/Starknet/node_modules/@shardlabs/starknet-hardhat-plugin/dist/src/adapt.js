...
exports.formatFelt = exports.adaptOutputUtil = exports.adaptInputUtil = void 0;
...
function isNumeric(value) {
    ...
    return decimalRegex.test(strValue) || constants_1.HEXADECIMAL_REGEX.test(strValue);
}
...
function validateAndConvertBooleanInput(value, errorMsg) {
    if (typeof value !== "boolean" && typeof value !== "number") {
        throw new starknet_plugin_error_1.StarknetPluginError(errorMsg);
    }
 ...
 */
function adaptInputUtil(functionName, input, inputSpecs, abi, isCairo1) {
    const adapted = [];
    ...
    for (let i = 0; i < inputSpecs.length; ++i) {
        ...
        if (COMMON_NUMERIC_TYPES.includes(inputSpec.type)) {
            const errorMsg = `${functionName}: Expected "${inputSpec.name}" to be a felt (Numeric); ` +
                `got: ${typeof currentValue}`;
            if (isNumeric(currentValue)) {
                adapted.push(toNumericString(currentValue));
            }
            else if (!isCairo1 && inputSpec.name.endsWith(constants_1.LEN_SUFFIX_DEPRECATED)) {
                const nextSpec = inputSpecs[i + 1];
                const arrayName = inputSpec.name.slice(0, -constants_1.LEN_SUFFIX_DEPRECATED.length);
                if (nextSpec &&
            ...
            const lenName = `${inputSpec.name}${constants_1.LEN_SUFFIX_DEPRECATED}`;
...
exports.adaptInputUtil = adaptInputUtil;
/**
...
function adaptComplexInput(input, inputSpec, abi, adaptedArray) {
    ...
    if (input === undefined || input === null) {
        throw new starknet_plugin_error_1.StarknetPluginError(`${inputSpec.name} is ${input}`);
    }
    ...
    if (isTuple(type)) {
        ...
        if (isNamedTuple(type)) {
            ...
            if (inputLen !== memberTypes.length) {
                const msg = `"${inputSpec.name}": Expected ${memberTypes.length} member${memberTypes.length === 1 ? "" : "s"}, got ${inputLen}.`;
                throw new starknet_plugin_error_1.StarknetPluginError(msg);
            }
...
function adaptStructInput(input, inputSpec, abi, adaptedArray) {
    const type = inputSpec.type;
    if (!(type in abi)) {
        throw new starknet_plugin_error_1.StarknetPluginError(`Type ${type} not present in ABI.`);
    }
 ...
 */
function adaptArray(result, resultIndex, arrayType, abi) {
    const elementType = arrayType.slice(ARRAY_TYPE_PREFIX.length, arrayType.length - ARRAY_TYPE_SUFFIX.length);
    const adaptedArray = [];
 ...
 */
function adaptOutputUtil(rawResult, outputSpecs, abi) {
...
exports.adaptOutputUtil = adaptOutputUtil;
/**
...
function formatFelt(value) {
    ...
    if (shiftedAbs < 2 ** 100)
        return starknet_1.num.toHex(shiftedValue);
    return starknet_1.num.toHex(value);
}
exports.formatFelt = formatFelt;
