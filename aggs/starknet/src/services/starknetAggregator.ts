...
export class StarknetAggregator {
  ...
  // Find the most optimized path between startToken and endToken using the mock pools
  async findOptimalPath(
    ...
    try {
      const paths = await session.readTransaction((tx) =>
        this.findPaths(tx, startToken, endToken, inputAmount, minLiquidity, maxPaths)
      );
      return paths;
    } finally {
      await session.close();
  ...
  // Query Neo4j to find paths based on mock pool relationships
